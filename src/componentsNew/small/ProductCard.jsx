...
//   return (
//     <div className="p-1 rounded-[15px] group shadow w-fit relative overflow-hidden bg-cover bg-center">
...
//                 src={img}
//                 alt={`slide-${index}`}
//                 className="md:w-[200px] w-full h-[160px] rounded-[15px] object-cover flex-shrink-0"
//               />
//             ))}
//           </div>
//         </div>
...
const ProductCard = ({ images }) => {
  ...
  // Scroll to selected slide when `current` changes
  useEffect(() => {
    if (imgRef.current) {
      imgRef.current.scrollTo({
        left: current * imgRef.current.clientWidth,
        behavior: "smooth",
      });
    }
  }, [current]);

  // Detect scroll position and update indicator
  const handleScroll = () => {
  ...
  return (
    <div className="p-1 rounded-[15px] group shadow w-fit relative overflow-hidden bg-cover bg-center">
      ...
      <div className="relative">
        {/* Scrollable wrapper */}
        <div
          className="md:w-[200px] w-full overflow-x-auto rounded-[15px] scroll-smooth snap-x snap-mandatory no-scrollbar"
          ref={imgRef}
          ...
          <div className="flex">
            {images.map((img, index) => (
              <img
                ...
                src={img}
                alt={`slide-${index}`}
                className="md:w-[200px] w-full h-[160px] rounded-[15px] object-cover flex-shrink-0 snap-start"
              />
            ))}
          </div>
        </div>
      ...
      <div className="w-full flex items-center justify-between md:px-3 px-1 py-2">
        ...
        <div
          className="rounded-full p-3 bg-[#f0f0f0] cursor-pointer hover:bg-[#ffecee]"
